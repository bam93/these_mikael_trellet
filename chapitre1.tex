...
La complexité de l'environnement n'est pas seulement dépendant de sa composition et de la richesse de sa représentation mais également du degré de précision choisi pour le modéliser. Ainsi, il est possible d'utiliser un environnement plus ou moins détaillé selon la capacité de calcul à disposition et la précision recherchée. Il existe par exemple plusieurs modèles permettant de considérer les 3 atomes de la molécules d'eau (H20) non plus comme 3 atomes indépendants mais comme une seule sphère pondérant les propriétés physico-chimiques des 3 atomes ensemble. Selon le niveau de détail désiré l'approximation des valeurs des propriétés physico-chimiques utilisées dans les équations mathématiques de la simulation numérique sera plus ou moins importante. Une plus grande approximation entrainera une précision moindre mais des temps de calcul réduit permettant l'étude de complexes moléculaires de grande taille. Ceci est illustré dans la figure X.

Lorsque la structure de départ et l'environnement ont été défini la simulation moléculaire peut commencer. Cette simulation consiste à donner une énergie de départ au système moléculaire (protéine + environnement) afin de le mettre en mouvement. Chaque déplacement atomique de la simulation est régit par des équations mathématiques correspondant aux lois physiques connus s'appliquant habituellement dans une cellule. Des paramètres physiques comme la température, la pression ou les forces d'attraction et de répulsion atomiques sont appliquées et permettra au système d'évoluer vers différents niveaux énergétiques. Le but de la simulation moléculaire est de faire parcourir à la structure étudiée le paysage énergétique propre à la structure afin de trouver le minimum énergétique global le plus bas, et donc le plus stable pour cette structure. Cet état de stabilité doit normalement refléter l'état de la protéine étudiée dans son environnement naturel cellulaire. Tout au long de la simulation, les coordonnées 3d de la structure sont sauvegardées afin de pouvoir rejouer la trajectoire de la simulation à tout moment au moyens de programmes de visualisation 3d.
...
Les programmes de visualisation moléculaire 3d permettent de visualiser des fichiers de coordonnées 3d de molécules dans un espace de visualisation adapté permettant de mettre en avant certaines de leurs particularités structurelles. Ces programmes s'appuient sur différents formats de fichier pour ces coordonnées même si la plupart sont capables de lire au moins le format PDB.
