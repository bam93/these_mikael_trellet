...
\begin{figure}[h]
  ...
  {\includegraphics[width=0.9\linewidth]{./figures/ch1/cellule}}
    \caption[Cellule eucaryote]{\it Cellule eucaryote en coupe horizontale.
    \commentaire{MB: citer la source de cette image.}}
    \label{Fig:cellule}
  \hspace{0.2cm}
\end{figure}
...
Le fonctionnement d'une cellule vivante implique aussi d'autres acteurs. Parmi ces molécules, les \textbf{polysaccharides} \commentaire{MB: peut-etre ajouter un lien ici ou ailleurs vers le site glycopedia maintenu par S Perez qui donne une riche mine d'infos pour les sucres} et les \textbf{lipides} \commentaire{MB: en analogie a glycopedia on pourrait citer eg http://www.lipidmaps.org/} ne sont pas générées par le code génétique mais jouent un rôle prépondérant dans la structuration, notamment de la membrane cellulaire (voir Figure \ref{Fig:cellule}), d'autres stockent l'énergie nécessaire à la cellule et enfin certaines fonctionnent comme messagers inter- et intra-cellulaires.
...
\begin{figure}
  ...
  (b) Représentation simplifiée de l'ADN en 3d, chaque type de nucléotides étant représenté par une couleur différente.
    \commentaire{MB: citer la source de ces images. C'est le cas pour tous les images venu d'ailleurs dans toute la thèse; je ne le répéterai pas par la suite.}}
  % \hspace{0.2cm}
\end{figure}
...
\begin{figure}[htb]
 ...
 {\includegraphics[width=0.6\linewidth]{./figures/ch1/chromosome_adn_horizontal.pdf}}
   \caption[Echelles de structuration du chromosome à l'ADN]{Les différentes échelles de structuration d'un chromosome, forme condensée de l'ADN. Ce chromosome est lui-même composé d'ADN enroulé autour de protéines structurantes, les histones.}
...
Ces acides aminés possèdent une partie commune, le squelette, et une partie spécifique appelée la \textit{chaîne latérale}, qui caractérise le type d'acide aminé. C'est au niveau de la partie commune que les acides aminés sont liés par une \textit{liaison peptidique}, la séquence des parties communes constituant la \textit{chaîne principale} (ou squelette) de la protéine (cf. Figure \ref{Fig:amino_acid_structure} et \ref{Fig:peptide_bond}). 
...
Enfin, les protéines possèdent également des motifs plus importants, souvent le résultat de l'agencement dans l'espace des motifs de structures secondaires cités précédemment. C'est la \textbf{structure tertiaire} ou structure tridimensionnelle de la protéine (cf. Figure \ref{Fig:secondary_structure}). 
...
\begin{figure}[htb]
  ...
  {\includegraphics[width=0.75\linewidth]{./figures/ch1/cell_membrane.pdf}}
    \caption[Schéma d'une membrane cellulaire.]{\it Schéma d'une membrane cellulaire composée d'une double couche lipidique sur laquelle sont liés différents types de protéines et des polysaccharides (ou glucides).}
...
\commentaire{MB: manque la figure suivante!}
La Figure \ref{Fig:schema_seq_bio_struct} donne un aperçu de ce découpage et identifie 3 grandes tâches : la collecte d'informations structurelles \textit{via} des techniques expérimentales et théoriques, la modélisation de structures à partir des informations récoltées et la simulation moléculaire qui permet d'ajouter une dimension dynamique indispensable pour l'étude de phénomènes et enfin l'analyse et l'interprétation de résultats permettent de postuler des hypothèses quant aux phénomènes étudiés et d'affiner les étapes de modélisation.
...
\commentaire{MB: pas d'accord sur la gamme de resolutions citees dans la phrase suivante; on trouve bien des structures a 1A de resolution, voire mieux, et on a des prot. membranaires au-dela de 3. A mettre a jour.}
Suivant les biomolécules observées, il est possible d'obtenir des structures tridimensionnelles à des résolutions comprises entre 2 et 3\r{A} pour une précision sur la position des atomes autour de 0.5\r{A}. 1\r{A} constituant une précision atomique. Les cartes de densité et les modèles atomiques créés à partir de ces cartes sont illustrés dans la Figure \ref{Fig:resolution_xray}.
...
\begin{figure}
  ...
  \begin{subfigure}{.5\textwidth}
  \centering
  {\includegraphics[width=0.6\linewidth]{./figures/ch1/cryoem_density.png}}
    \label{Fig:cryoem_density}
  \end{subfigure}
  \hspace{0.2cm}
  \caption[(a) Schéma simplifié illustrant la technique de cryo-microscopie électronique. (b) Carte de densité électronique d'un complexe protéique.]{\it (a) Schéma simplifié illustrant la technique de cryo-microscopie électronique. Les électrons traversent l'échantillon prisonnier de la glace avant d'atteindre une lentille magnétique qui va les modifier et les projeter sur un scintillateur permettant d'obtenir une image similaire à celle présentée en bas.
  (b) Carte de densité (en rendu surfacique transparent blanc) d'un complexe protéique (GroEL-GroES) composé de multiples sous-unités et faisant parti de la famille des protéines chaperonnes chez les bactéries.}
\end{figure}
...
\commentaire{MB: je ne peux etre d'accord avec cette formulation, trop negative a mon sens; certes la modelisation a des limites, mais les approches exp. egalement. Prend ex. le boulot de DE Shaw Science 2010 qui etablit bien la maturite des approches si le bon champs de force est utilise et on dispose d'un temps de simulation suffisant.}
...
\commentaire{Fossé? Tu es vraiment tres critique concernant la modelisation; tu fais aussi abstraction que et la RMN et la cristallo ne peuvent se faire sans affinement avec dynamique moleculaire - certes fortement guide/contraint par les mesures experimentales, mais je pense que cela illustre bien qu'il n'y a point de fossé.}
...
et les méthodes de Monte-Carlo qui minimisent l'énergie potentielle d'un système moléculaire par changement successifs aléatoires, se reposent sur ces lois physico-chimiques décrites dans chaque champs de force (voir section \ref{simu}). 
...
\begin{figure}
  ...
  {\includegraphics[width=0.75\linewidth]{./figures/ch1/harmonic_vs_morse_potential.pdf}}
    \caption[Contributions énergétiques des liaisons covalentes.]{\it Contributions énergétiques des liaisons représenté par un potentiel harmonique (en vert) ou un potentiel de Morse (en bleu) en fonction de la distance entre deux atomes.}
...
\begin{figure}
  ...
  {\includegraphics[width=0.75\linewidth]{./figures/ch1/coarse_grain_ff.pdf}}
    ...
    \commentaire{MB: l'eau en vert.. rarement vu; un choix plus conventionnel serait soit rouge soit bleu pour l'oxygene de l'eau.}
...
\subsubsection{Repliement moléculaire (folding) ou prédiction de structure tertiaire} 
...
Lorsque la structure secondaire est connue, il est possible d'effectuer une simulation moléculaire afin d'utiliser des paramètres physico-chimiques pour diriger le repliement des motifs de structures secondaires identifiés. Les changements conformationnels menant de la structure de départ au modèle 3d final sont souvent importants puisque aucune information préalable n'est utilisée. Il est ainsi courant d'utiliser un champ de force gros grain pour réduire les temps de calcul et permettre un parcours plus complet du paysage énergétique de la protéine. \commentaire{MB: euh.. courant, non, la plupart des champs de force gros grain sont incapable de faire du repliement car on leur impose une strucutre de reference, e.g. un modele de ressorts dans Martini; quelques-uns, plutot rares, comme OPEP sous certaines conditions, sont capables de faire du repliement seulement}
...
\begin{figure}
  ...
  \hspace{0.2cm}
  \end{subfigure}%
 \caption[Modèles artistiques d'une phéromone et d'un brin d'ADN.]{\it (a) Modèle en bois de Julian Voss-Andreae illustrant la chaîne principale d'une phéromone.
...
Les logiciels de visualisation permettent principalement de mettre en avant certaines des particularités structurelles des molécules au moyen de jeux de couleurs ou de formes largement adoptés dans la communauté et considérés comme des standards.
\commentaire{MB: je te trouve trop optimiste sur ces "standards"; il y a quand meme une grande variablite, notamment au niveau des codes couleurs}
...
\begin{itemize}
  ...
  \commentaire{MB: ce que tu decris semble plutot correspondre au mode "trace" qui relie les atomes carbon alpha; licorice, en general, montre tous les atomes, mais en batonnets uniquement}
  ...
  \item Représentation par \textbf{rubans} : de la même manière que la représentation par licorice \commentaire{?trace?}, cette représentation suit les carbones alpha composant la chaîne principale de la protéine et affiche un ruban large le long de la chaîne principale.
